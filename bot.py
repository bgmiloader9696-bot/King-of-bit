...
async def check_cooldown(user_id):
    ...
    if user_id in user_last_click:
        if now - user_last_click[user_id] < 1.5:
            return False
...
class PredictionEngine:
    ...
    def analyze_zigzag(colors):
        ...
        for i in range(min(7, len(colors)-1)):
            if colors[i] != colors[i+1]:
                zz += 1
        if zz >= 6:
            return {'vote':('R' if colors[0]=='G' else 'G'), 'weight':3.5, 'score':zz}
        if zz >= 4:
            return {'vote':('R' if colors[0]=='G' else 'G'), 'weight':1.8, 'score':zz}
        if zz <= 1:
            return {'vote':colors[0], 'weight':1.5, 'score':zz}
    ...
    def analyze_balance(colors, window):
        ...
        if window == 6:
            if g >= 5:
                return {'vote':'R','weight':3.2}
            if r >= 5:
                return {'vote':'G','weight':3.2}
            if g == 4:
                return {'vote':'R','weight':1.8}
            if r == 4:
                return {'vote':'G','weight':1.8}
            return {'vote':('R' if colors[0]=='G' else 'G'), 'weight':0.6}
        elif window == 10:
            if g >= 8:
                return {'vote':'R','weight':2.5}
            if r >= 8:
                return {'vote':'G','weight':2.5}
            if g >= 7:
                return {'vote':'R','weight':1.5}
            if r >= 7:
                return {'vote':'G','weight':1.5}
            if g > r+1:
                return {'vote':'R','weight':1.0}
            if r > g+1:
                return {'vote':'G','weight':1.0}
            return {'vote':('R' if colors[0]=='G' else 'G'), 'weight':0.4}
        else:
            if g >= 16:
                return {'vote':'R','weight':2.0}
            if r >= 16:
                return {'vote':'G','weight':2.0}
            if g > r+3:
                return {'vote':'R','weight':1.2}
            if r > g+3:
                return {'vote':'G','weight':1.2}
    ...
    def analyze_momentum(colors):
        ...
        if g_new >= 3 and g_old <= 1:
            return {'vote':'R','weight':2.5}
        if g_new <= 0 and g_old >= 2:
            return {'vote':'G','weight':2.5}
        if g_new > g_old + 1:
            return {'vote':'R','weight':1.3}
        if g_old > g_new + 1:
            return {'vote':'G','weight':1.3}
        return {'vote':('R' if colors[0]=='G' else 'G'), 'weight':0.4}
    ...
    def analyze_patterns(colors):
        ...
        if len(colors) >= 3:
            ...
            if t == 'GGG':
                signals.append({'vote':'R','weight':3.0})
            elif t == 'RRR':
                signals.append({'vote':'G','weight':3.0})
            elif t == 'GRG':
                signals.append({'vote':'R','weight':2.0})
            elif t == 'RGR':
                signals.append({'vote':'G','weight':2.0})
            elif t == 'GGR':
                signals.append({'vote':'G','weight':1.2})
            elif t == 'RRG':
                signals.append({'vote':'R','weight':1.2})
            else:
                signals.append({'vote':('R' if colors[0]=='G' else 'G'),'weight':0.5})
        return signals
    ...
    def should_skip(agg, streak):
        if streak >= 2:
            return True, f"LOSS ×{streak}"
        if agg['agree_ratio'] < 0.52:
            return True, "SPLIT"
        if agg['margin'] < 1.5:
            return True, "LOW MARGIN"
        return False, ""

    async def get_prediction(self, mode):
        try:
            ...
            if not items or len(items) < 6:
                # Fallback to random
                period = str(random.randint(1000, 9999))
                return {
                    'period': period,
                    'prediction': random.choice(['GREEN', 'RED', 'SKIP']),
            ...
            signals = []
            s1 = self.analyze_streak(colors)
            signals.append(s1)
            s2 = self.analyze_zigzag(colors)
            signals.append(s2)
...
class TelegramBot:
    ...
    async def button_handler(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if not await check_cooldown(uid):
            return
        ...
        if query.data == "start_prediction":
            ...
            pred = await self.engine.get_prediction(mode)
            if pred:
            ...
            if hist:
                for h in reversed(hist):
                    if h['result'] == 'WIN':
                        ...
                        text += f"`{h['period']}` ⏳ {h['result']} [{h['mode']}]\n"
