    ...
    async def predict(self, mode):
        try:
            ...
            if not items or len(items) < 6:
                ...
                next_period = str(int(datetime.now().timestamp()) % 10000)
                return {
                    'period': next_period[-4:],
            ...
            # Fallback prediction
            return {
                'period': str(int(time.time()) % 10000),
                'prediction': random.choice(['GREEN', 'RED']),
                'confidence': random.randint(65, 92),
                'streak': 1,
                'pattern': 'MIXED'
...
class TelegramBot:
    ...
    async def button_handler(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if query.data == "start_prediction":
            ...
            pred = await self.engine.predict(mode)
            if pred:
