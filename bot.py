...
load_dotenv()
BOT_TOKEN = "8723966859:AAFH68kkE6ac8MjiBmQy8Qk21Hqt5Ovod_k"  # YAHAN APNA BOT TOKEN DALO
ADMIN_ID = 7983241359  # YAHAN APNA ADMIN ID DALO
...
pending_requests = {}
approved_users = set()
user_data = {}  # Stores user settings, history, and stats
running_tasks = {}
user_cooldown = {}
...
def prevent_double_click(func):
    ...
    async def wrapper(update: Update, context: ContextTypes.DEFAULT_TYPE, *args, **kwargs):
        ...
        now = time.time()
        if user_id in user_cooldown and now - user_cooldown[user_id] < 1.5:
...
class TelegramBot:
    ...
    async def button_handler(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        # Check approval for other actions
        ...
        if query.data == "start_prediction":
            ...
            try:
                ...
                if pred:
                    if pred['prediction'] == 'SKIP':
                        ...
                        msg = f"⚡ *{mode} STARTED*\n\nFirst Prediction:\n`{pred['period']}` {emoji} {pred['prediction']} ({pred['confidence']}%)"
                    await query.edit_message_text(msg, parse_mode='Markdown')
                else:
                    await query.edit_message_text(f"⚡ *{mode} STARTED*\n\nWaiting for data...", parse_mode='Markdown')
            except:
                await query.edit_message_text(f"⚡ *{mode} STARTED*\n\nFetching predictions...", parse_mode='Markdown')
            ...
            hist = user_data[uid]['history'][-15:]  # Last 15 results
            ...
            if hist:
                for h in reversed(hist):
                    ...
                    # Format: PERIOD | MODE | RESULT | CONFIDENCE
                    confidence = h.get('confidence', 0)
                    if confidence > 0:
                        text += f"`{h['period']}` {emoji} {h['result']} [{h['mode']}] {confidence}%\n"
                    else:
                        text += f"`{h['period']}` {emoji} {h['result']} [{h['mode']}]\n"
                    
                # Show summary at bottom
                text += "\n━━━━━━━━━━━━━━━\n"
                text += f"✅ Wins: {user_data[uid]['wins']}  "
                text += f"❌ Losses: {user_data[uid]['losses']}  "
                text += f"⏭️ Skips: {user_data[uid]['skips']}"
            else:
                text += "No history yet. Start predictions to see results!"
            ...
            total = wins + losses
            total_predictions = user_data[uid].get('total_predictions', 0)
            ...
            if user_data[uid]['history']:
                for h in reversed(user_data[uid]['history']):
                    if h['result'] == 'WIN':
            ...
            text += f"├ ⏭️ Skips: {skips}\n"
            text += f"├ {'─'*20}\n"
            text += f"├ 📈 Win Rate: {win_rate:.1f}%\n"
            
            if streak > 0:
                streak_emoji = "🔥" if streak_type == "WIN" else "⚠️"
                text += f"├ {streak_emoji} Current Streak: {streak} {streak_type}\n"
            
            text += f"└ {'═'*20}"
            
            # Add mode-wise stats
            mode_stats = {}
            for h in user_data[uid]['history']:
                mode = h['mode']
                if mode not in mode_stats:
                    mode_stats[mode] = {'wins':0, 'losses':0, 'skips':0}
                if h['result'] == 'WIN':
                    mode_stats[mode]['wins'] += 1
                elif h['result'] == 'LOSS':
                    mode_stats[mode]['losses'] += 1
                elif h['result'] == 'SKIP':
                    mode_stats[mode]['skips'] += 1
            
            if mode_stats:
                text += "\n\n📊 *MODE WISE*\n"
                for mode, stats in mode_stats.items():
                    mode_emoji = "⚡" if mode == "30S" else "⏱️"
                    mode_total = stats['wins'] + stats['losses']
                    mode_rate = (stats['wins']/mode_total*100) if mode_total > 0 else 0
                    text += f"\n{mode_emoji} {mode}: {stats['wins']}W/{stats['losses']}L ({mode_rate:.0f}%)"
        ...
        while user_data.get(uid, {}).get(mode, False):
            try:
                ...
                if pred and pred['period'] != last_period:
                    ...
                    user_data[uid]['total_predictions'] = user_data[uid].get('total_predictions', 0) + 1
                    ...
                    if pred['prediction'] == 'SKIP':
                        ...
                        # Store as pending skip
                        result_status = 'PENDING_SKIP'
                    else:
                        emoji = "🟢" if pred['prediction'] == 'GREEN' else "🔴"
                        msg = (
                            f"🤖 *AI ENGINE*\n\n"
                    ...
                    if len(user_data[uid]['history']) > 100:
                        user_data[uid]['history'] = user_data[uid]['history'][-100:]
                
                # Wait for next cycle
                now = datetime.now()
                seconds = now.second
                wait = interval - (seconds % interval)
                await asyncio.sleep(wait + 2)
                
                # Check actual results after waiting
                await self.check_actual_results(uid, mode, context)
    ...
    async def check_actual_results(self, uid, mode, context):
        """Check actual results and update win/loss counts"""
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(API_URLS[mode]) as resp:
                    data = await resp.json()
            ...
            latest = items[0]
            actual_color = self.engine.get_color(latest)
            actual_period = str(latest.get('issueNumber', ''))[-4:]
            actual_number = latest.get('number', '?')
            
            # Find and update matching pending predictions
            updated = False
            for i, h in enumerate(user_data[uid]['history']):
                # Check if this is the period we're looking for and it's pending
                if h['period'] == actual_period and h['result'] in ['PENDING', 'PENDING_SKIP']:
                    
                    if h['prediction'] == 'SKIP':
                        # Skip doesn't affect win/loss
                        h['result'] = 'SKIP'
                        h['actual'] = actual_color
                        h['actual_number'] = actual_number
                        user_data[uid]['skips'] += 1
                        updated = True
                        logger.info(f"User {uid} - Period {actual_period}: SKIP confirmed")
                    
                    else:
                        # Check if prediction was correct
                        pred_color = 'G' if h['prediction'] == 'GREEN' else 'R'
                        if pred_color == actual_color:
                            h['result'] = 'WIN'
                            user_data[uid]['wins'] += 1
                            logger.info(f"User {uid} - Period {actual_period}: WIN 🟢")
                        else:
                            h['result'] = 'LOSS'
                            user_data[uid]['losses'] += 1
                            logger.info(f"User {uid} - Period {actual_period}: LOSS 🔴")
            ...
            # If we updated a result, send notification to user
            if updated:
                wins = user_data[uid]['wins']
                losses = user_data[uid]['losses']
                
                # Find the updated history item
                for h in user_data[uid]['history']:
                    if h['period'] == actual_period:
                        if h['result'] == 'WIN':
                            result_msg = f"✅ PERIOD {actual_period} RESULT: WIN! 🟢"
                        elif h['result'] == 'LOSS':
                            result_msg = f"❌ PERIOD {actual_period} RESULT: LOSS! 🔴"
                        else:
                            result_msg = f"⏭️ PERIOD {actual_period} RESULT: SKIP (as predicted)"
                        
                        notification = (
                            f"📊 *RESULT UPDATE*\n\n"
                            f"{result_msg}\n\n"
                            f"Actual: {'GREEN' if actual_color=='G' else 'RED'} ({actual_number})\n"
                            f"Your Prediction: {h['prediction']}\n\n"
                            f"Current Stats: {wins}W - {losses}L"
                        )
            ...
            logger.error(f"Error checking actual results: {e}")
...
if __name__ == "__main__":
    if BOT_TOKEN == "pvt token yahan dalo" or ADMIN_ID == 123456789:
        print("❌ Error: BOT_TOKEN and ADMIN_ID must be set in the code!")
        print("📝 Edit the bot.py file and put your token and admin ID")
        exit(1)
