...
load_dotenv()
BOT_TOKEN = "8723966859:AAFH68kkE6ac8MjiBmQy8Qk21Hqt5Ovod_k"  # YAHAN APNA BOT TOKEN DALO ( @BotFather se lo )
ADMIN_ID = 123456789  # YAHAN APNA ADMIN ID DALO ( @userinfobot se lo )
...
class TelegramBot:
    ...
    async def button_handler(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        # Admin approval - smooth handling
        ...
        # Handle other callbacks (start_prediction, mode_, etc.)
        if query.data == "start_prediction":
            ...
            if hist:
                for h in reversed(hist):
                    if h['result'] == 'PENDING':
                        text += f"`{h['period']}` ⏳ PENDING\n"
                    elif h['result'] == 'SKIP':
                        text += f"`{h['period']}` ⏭️ SKIP\n"
                    else:
                        emoji = "🟢" if h['result']=='WIN' else "🔴"
                        text += f"`{h['period']}` {emoji} {h['result']}\n"
            else:
                text += "No history yet\n"
            keyboard = [[InlineKeyboardButton("🔙 BACK", callback_data="main_menu")]]
            await query.edit_message_text(text, parse_mode='Markdown', reply_markup=InlineKeyboardMarkup(keyboard))
            ...
            if total>0:
                ...
                for h in reversed(user_data[uid]['history']):
                    if h['result']=='WIN': streak+=1
                    else: break
                text += f"Streak: {streak}W"
            keyboard = [[InlineKeyboardButton("🔙 BACK", callback_data="main_menu")]]
            await query.edit_message_text(text, parse_mode='Markdown', reply_markup=InlineKeyboardMarkup(keyboard))
    ...
    async def prediction_loop(self, uid, mode, context):
        ...
        while user_data.get(uid,{}).get(mode, False):
            try:
                ...
                await self.check_actual_result(uid, mode, context)
    ...
    async def check_actual_result(self, uid, mode, context):
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(API_URLS[mode]) as resp:
                    data = await resp.json()
            ...
            for i, h in enumerate(history):
                if h['period'] == actual_period and h['result'] == 'PENDING':
